...
dx_diff[:,0,1] = (dxana[:,6] - dxnum[:,0,1])
dx_diff[:,0,2] = (dxana[:,7] - dxnum[:,0,2])

dx_diff[:,1,0] = (dxana[:,9] - dxnum[:,1,0])
dx_diff[:,1,1] = (dxana[:,10] - dxnum[:,1,1])
dx_diff[:,1,2] = (dxana[:,11] - dxnum[:,1,2])

dx_diff[:,2,0] = (dxana[:,13] - dxnum[:,2,0])
dx_diff[:,2,1] = (dxana[:,14] - dxnum[:,2,1])
dx_diff[:,2,2] = (dxana[:,15] - dxnum[:,2,2])
output.PointData.append(dx_diff,"dx_diff")
...
time_step = 1.0e-3
output.PointData.append((tshift1 - tshift0)/time_step,"dtTargetShiftNumerical")
...
output.PointData.append(copy_dt_shift,"dtTargetShiftAnalytical")
